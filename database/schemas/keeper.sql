...
-- Volcando datos para la tabla keeper.almacen: ~0 rows (aproximadamente)
...
-- Volcando estructura para tabla keeper.forma_pago
...
-- Volcando datos para la tabla keeper.forma_pago: ~4 rows (aproximadamente)
INSERT INTO `forma_pago` (`id`, `nombre`, `descripcion`) VALUES
	(1, 'CONTADO', 'Pago inmediato al recibir la mercadería'),
	(2, 'CRÉDITO 30', 'Pago a 30 días calendario'),
	(3, 'CRÉDITO 60', 'Pago a 60 días calendario'),
	(4, 'CRÉDITO 90', 'Pago a 90 días calendario');
...
CREATE TABLE IF NOT EXISTS `ingreso` (
  ...
  `razon_social_factura` varchar(255) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `inventario` (
  ...
  KEY `idx_inventario_stock_bajo` (`id_producto`,`id_almacen`,`stock_actual`,`stock_minimo`),
  ...
  CONSTRAINT `chk_inventario_stock_maximo` CHECK (`stock_maximo` is null or `stock_maximo` >= `stock_minimo`),
  CONSTRAINT `chk_inventario_stock_reservado` CHECK (`stock_reservado` >= 0 and `stock_reservado` <= `stock_actual`),
...
CREATE TABLE IF NOT EXISTS `proveedor` (
  ...
  KEY `fk_proveedor_forma_pago` (`id_formapago`),
  ...
  CONSTRAINT `fk_proveedor_forma_pago` FOREIGN KEY (`id_formapago`) REFERENCES `forma_pago` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ...
  CONSTRAINT `chk_proveedor_limite_credito` CHECK (`limite_credito` >= 0),
  CONSTRAINT `chk_proveedor_tiempo_entrega` CHECK (`tiempo_entrega` is null or `tiempo_entrega` > 0),
  CONSTRAINT `chk_proveedor_calificacion` CHECK (`calificacion` is null or `calificacion` between 1 and 5)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Volcando datos para la tabla keeper.proveedor: ~0 rows (aproximadamente)
...
-- Volcando datos para la tabla keeper.proveedor_email: ~0 rows (aproximadamente)
...
-- Volcando datos para la tabla keeper.proveedor_telefono: ~0 rows (aproximadamente)
...
-- Volcando datos para la tabla keeper.sessions: ~0 rows (aproximadamente)
...
-- Volcando datos para la tabla keeper.unidad_medida: ~6 rows (aproximadamente)
...
-- Volcando datos para la tabla keeper.usuario: ~2 rows (aproximadamente)
INSERT INTO `usuario` (`id`, `username`, `email`, `password_hash`, `nombre_completo`, `rol`, `estado`, `ultimo_acceso`, `telefono`, `ci`, `departamento`, `created_at`, `updated_at`) VALUES
	...
	(2, 'admin1', 'admin@example.com', '$2b$10$mhdawduXKKiHkl0Pj9498.ZjsAehaANbTv7BIh4wkKtpmnnYdmFIO', NULL, 'ADMIN', 'ACTIVO', NULL, NULL, NULL, NULL, '2025-09-26 15:53:20', '2025-09-26 15:53:20'),
	(3, 'admin2', 'admin2@example.com', '$2b$10$1XI8jPOTXJhbsv4BpBF6ae7KG07y0gvKvsXmoKGEGk8xRopLEn/.C', NULL, 'ADMIN', 'ACTIVO', '2025-09-26 16:02:54', NULL, NULL, NULL, '2025-09-26 16:00:13', '2025-09-26 16:02:54');
